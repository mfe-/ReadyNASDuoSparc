...
#define WAIT_WORSTCASE	(40*HZ)	/* 40sec  - worst case when spinning up */
//#define WAIT_CMD	(10*HZ)	/* 10sec  - maximum wait for an IRQ to happen */
//#define WAIT_CMD	(30*HZ)	/* 30sec  - maximum wait for an IRQ to happen */
#define WAIT_CMD	(40*HZ)	/* 40sec  - maximum wait for an IRQ to happen */
...
//	u8	(*ratefilter)(ide_drive_t *, u8);
	...
	unsigned dma;
#ifdef CONFIG_PADRE
#define ALLOW_MASTER_SECTOR	1
        byte             true_device;    /* If this hwif is for real? */
        byte             state0; 
        byte             state1;
        byte             state2;
#endif
...
#ifdef CONFIG_PADRE
//This is not a good, think again later.
typedef ide_hwif_t padre_hw_ideif_t[MAX_HWIFS];
#endif
